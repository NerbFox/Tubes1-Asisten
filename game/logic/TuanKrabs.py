...
class GradientLogic(BaseLogic):
    ...
    def base_fn(
        self, inp: Position, distance_offset: int
    ):  # untuk mengatur pergerakan bot (tidak termasuk pergerakan terhadap teleporter)

        total = 0.0
        inventory_filled = float(self.bot.properties.diamonds) / float(
            self.inventory_size
        )
        ...
        redButton_target: list[GameObject] = list(
            filter(
                lambda x: x.type == "DiamondButtonGameObject", self.board.game_objects
            )
        )
        ...
        for bot in bots:
            ...
            if (
                distance > 2
            ):  # Jika jarak bot lain lebih dari 2, maka bot akan berjalan seperti biasa
                continue
            ...
            if inventory_filled > 3 / 4:
                ...
                inventory_filled < 1 / 4
            ):  # Jika inventory terisi kurang dari 1/4 , maka bot akan berusaha memakan bot lain jika memungkinkan

                other_base_distance = manhattan_distance(bot.properties.base, pos)
                if other_base_distance < 8:
                    other_inventory = float(bot.properties.diamonds) / float(
                        self.inventory_size
                    )
    ...
    def fn(
        ...
        teleporter: list[GameObject] = list(
            filter(lambda x: x.type == "TeleportGameObject", self.board.game_objects)
        )
    ...
    def next_move(
        ...
        if hasattr(bot_provider, "config") and hasattr(
            bot_provider.config, "inventory_size"
        ):
