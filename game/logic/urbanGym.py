...
class BotsMove(BaseLogic):
    ...
    def get_way(
        ...
        if not checktele:
            ...
            if next_pos_x == telestart.x and next_pos_y == telestart.y:
                ...
                if (next_pos_x == telestart.x and current_y + 1 == telestart.y) and (
                    next_pos_y != telestart.x
                ):
                    delta_y = 0
                else:
                    delta_x = 0
                ...
                if (next_pos_x == teletarget.x and current_y + 1 == teletarget.y) and (
                    next_pos_y != teletarget.x
                ):
                    delta_y = 0
                else:
                    delta_x = 0
    ...
    def get_tacklebot(self, board: Board, board_bot):
        ...
        for a in item:
            ...
            if (
                a.type == "BotGameObject"
                and a.properties.can_tackle
                and a.position != board_bot.position
            ):
                tacklers.append(a)
        return tacklers
            ...
            if (checkred) and (checktele):
                ...
                if item.type == "DiamondButtonGameObject":
                    ...
                    teleports.append(
                        (
                            abs(
                                abs(item.position.x - current_position.x)
                                + abs(item.position.y - current_position.y)
                            ),
                            item.position,
                        )
                    )
            ...
            for l in range(block_start_col, block_end_col):
                for diamond in diamond_objects:
                    if (diamond.position.x == k) and (diamond.position.y == l):
                        ...
                        listrangediamond.append(
                            (
                                abs(
                                    abs(diamond.position.x - current_position.x)
                        ...
                        for diamond in diamond_objects:
                            if (diamond.position.x == k) and (diamond.position.y == l):
                                ...
                                listrangediamond.append(
                                    (
                                        abs(
                                            abs(diamond.position.x - start_position.x)
                                            + abs(diamond.position.y - start_position.y)
                                        ),
                                        diamond.position,
                                    )
                                )
                sorted_listrangediamond = sorted(
                    listrangediamond, key=lambda x: x[0]
                )  # sort berdasar jarak start position
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        current_position = board_bot.position
        distanceToBase = abs(
            abs(base.x - current_position.x) + abs(base.y - current_position.y)
        )
        ...
        if (
            ...
            or props.diamonds == 4
            or (
                (distanceToBase + 1 == (props.milliseconds_left / 1000))
                and props.diamonds != 0
            )
            or (distanceToBase == (props.milliseconds_left / 1000))
        ):  # error pas inventory diamond = 4 terus dapet diamond merah
            # Move to base
            ...
            for bot in bots:
                if (
                    bot.position.x // 5 == current_position.x // 5
                ) and bot.position.y // 5 == current_position.y // 5:
            ...
            totalpointblock, listrangediamond = self.current_totalpointblock(
                current_position, diamond_objects
            )
            total_teletarget, _ = self.current_totalpointblock(
                teletarget, diamond_objects
            )
            # kondisi jika total point di block kita 0
            if totalpointblock == 0:  ##bagian red buttoon

                # kondisi kalo ada red button terdekat
                if (redpos.x // 5 == current_position.x // 5) and (
                    redpos.y // 5 == current_position.y // 5
                ):
                    self.goal_position = redpos
                # kondisi jika ada teleport terdekat dan point teleport tujuannya banyak
                elif (
                    (telestart.x // 5 == current_position.x // 5)
                    and (telestart.y // 5 == current_position.y // 5)
                ) and (total_teletarget != 0):
                    checktele = True
                    self.goal_position = telestart
                # kondisi mencari ke block lain
                else:
                    if props.diamonds >= 2:
                        self.goal_position = self.closestDiamond(
                            current_position, diamond_objects, board
                        )
                    else:
                        self.goal_position = self.totalpointblock(
                            current_position, diamond_objects
                        )
